# 基于1-QueryRewriting.py增加了 API Key 校验、异常处理、模型输出解析和容错机制
# Query改写使用示例
# 导入依赖库
import dashscope
import os
...
if not dashscope.api_key:
    raise Exception("未检测到环境变量 DASHSCOPE_API_KEY！\n"
                    "设置方式(Windows PowerShell)：$env:DASHSCOPE_API_KEY='你的通义千问API-KEY'")

# 基于 prompt 生成文本（增加接口异常容错）
def get_completion(prompt, model="qwen-turbo"):
    ...
    try:
        ...
        if response.status_code != 200:
            raise Exception(f"大模型接口调用失败：code={response.status_code}, message={response.message}")
        # 校验output与choices非空
        if not response.output or not response.output.choices:
            raise Exception("大模型返回output为空，无生成内容")
...
### 分解后的问题列表 ###
请严格只输出JSON数组，例如：["问题1", "问题2", "问题3"]，不要额外文字
"""
